...
import UserData.FileStorage.FileUploader;
import UserData.FileStorage.FileUploaderFactory;
import UserData.UserData;
import UserData.DeleteData.HardDelete;
import UserData.DeleteData.SoftDelete;
import UserData.compressFile.ExecutorCompress;
import activity.IUserActivityService;
import activity.UserActivity;
import activity.UserActivityService;
import UserData.docs.ExecutorDocs;
...
public class Application {
    ...
    public static void main(String[] args) throws SystemBusyException, BadRequestException, NotFoundException {
        ...
        DataExporter dataExporter = new DataExporter(
                new ExportIamData(userService),
                new ExportActivityData(userActivityService),
                new ExportPaymentData(paymentService),
                new ExportPostData(postService)
        );




        UserData userData = new UserData(dataExporter,userService);
        userData.start(userName);



        ExecutorDocs executorDocs = new ExecutorDocs(userService);
        executorDocs.makePDF(userName);

        ExecutorCompress executorCompress = new ExecutorCompress(userService);
        executorCompress.start(userName);


/*
        FileUploaderFactory fileUploaderFactory = new  FileUploaderFactory();
        FileUploader dropBox = fileUploaderFactory.getUploader("DROPBOX");
        dropBox.uploadFile(userName);




        SoftDelete softDelete = new SoftDelete();
        softDelete.deleteUserData(userName);

        HardDelete hardDelete = new HardDelete();
        hardDelete.deleteUserData(userName);
